...
def build_network_design_graph(model: ChatOpenAI, checkpointer: AsyncRedisSaver):
    ...
    async def build_fortigate_handoff(state: NetworkDesignState) -> NetworkDesignState:
            ...
            [
                SystemMessage(
                    content=(
                        ...
                        "security_zones, firewall_policy_intent, nat_requirements, vpn_requirements, logging_requirements, ha_requirements, "
                        "change_window, rollback_expectations, and additional_context. This is a handoff only, not a device change."
                    )
                ),
                HumanMessage(
                    content=json.dumps(
        ...
        for key in ("wan_circuits", "lan_networks", "security_zones"):
            value = handoff.get(key)
            if value in (None, ""):
                handoff[key] = []
